...
As an MCP implementation, EMCP supports all the core server features from the [[https://modelcontextprotocol.io/specification/2025-11-25/server][2025-11-25 version of the MCP specification]] and even has (as of yet unused) facilities for server-to-client notifications and requests. Regarding actual user-facing functionality, I am adding tools etc. to the project as I need them and find a usable design for them. So while I have plans for more tools, for now only the simple ones are in the repository and more complex ones like code execution do not exist yet (even though I teased it in the previous paragraph). However, if you really want a code execution tool /right now/, EMCP has been designed to be extensible and adding it yourself is almost trivial. Just [[#custom-components][follow the instructions]].

* Installation

Use the recipe =(emcp :fetcher git :url "https://codeberg.org/martenlienen/emcp")= to install EMCP, for example with ~package-vc-install~. =use-package= with [[https://github.com/progfolio/elpaca][elpaca]] would be
#+begin_src emacs-lisp
  (use-package emcp
    :ensure (emcp :fetcher git :url "https://codeberg.org/martenlienen/emcp"))
#+end_src
...
For other cases, call ~emcp-start~ interactively, which asks for a profile and places the server URL on the kill-ring. ~emcp-stop~, ~emcp-restart~ and ~emcp-reload~ do what you would expect them to do and are useful when you develop [[*Custom MCP Components][your own MCP components]] or custom profiles.
...
Search for symbols matching a regular expression, optionally restricted to a specific kind.
...
Find the definition of a symbol, returning its location and source code.
