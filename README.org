...
#+caption: Claude Code in agent-shell using EMCP to develop =emcp-session-manager=
#+attr_html: :width 640
[[./media/demo.webp]]
...
The following snippet should, in theory, be enough to integrate EMCP into gptel if you have already activated gptel's MCP integration:
...
However, as of June 14, 2026 [[https://github.com/lizqwerscott/mcp.el][mcp.el]] only supports the =2024-11-05= and =2025-03-26= versions of the MCP specification and EMCP implements =2025-11-25=.
...
Start the session manager with =M-x emcp-session-manager=. It gives you an overview over all running servers and their sessions, so that you can easily see which agent in which project is connected to EMCP. From the manager view, you can also change the policies of the =eval= and =send-keys= tools, so if you have put a session in accept mode, which accepts all code evaluations and does not ask for your input anymore, you can return a session to the default ask mode. Press =l= to jump to a server's log buffer or =k= to kill a server you no longer need.

#+caption: The session manager lets you inspect the sessions and update their policies
#+attr_html: :width 640
[[./media/session-manager.webp]]
...
# begin:prompts
*** =/screenshot= [[./emcp-screenshot.el::164][~emcp-prompt-screenshot~]]
:PROPERTIES:
...
*** =eval= [[./emcp-tools-eval.el::202][~emcp-tools-eval~]]
:PROPERTIES:
:CUSTOM_ID: tool-eval
:END:
...
*** =screenshot= [[./emcp-screenshot.el::144][~emcp-tools-screenshot~]]
:PROPERTIES:
...
*** =send-keys= [[./emcp-tools-send-keys.el::148][~emcp-tools-send-keys~]]
:PROPERTIES:
...
** [[./emcp-confirm.el::224][~emcp-confirm-notify-darwin-bundle-id~]]
:PROPERTIES:
...
** [[./emcp-confirm.el::282][~emcp-confirm-notify-function~]]
:PROPERTIES:
...
** [[./emcp-tools-eval.el::42][~emcp-tools-eval-default-policy~]]
:PROPERTIES:
